...
import com.allprice.app.databinding.ActivityMainBinding
import java.io.File
...
class MainActivity : AppCompatActivity() {
    ...
    private fun startLocalServer() {
        setStatus(getString(R.string.connecting), R.color.warning, R.drawable.dot_yellow)
    ...
    private fun pollStatus() {
        ...
        val raw = try {
            val f = File(filesDir, "allprice_status.txt")
        ...
        when {
            text.startsWith("server_ready") -> {
                setStatus(getString(R.string.connection_success), R.color.success, R.drawable.dot_green)
                binding.offlineBanner.visibility = View.GONE
                binding.progressBar.visibility = View.VISIBLE
                binding.webView.loadUrl("http://127.0.0.1:8001/")
                frontendLoaded = true
                return
            }
            text.startsWith("error_") -> {
                setStatus(getString(R.string.connection_failed), R.color.error, R.drawable.dot_red)
